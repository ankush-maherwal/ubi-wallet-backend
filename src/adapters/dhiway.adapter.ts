...
export class DhiwayAdapter implements IWalletAdapterWithOtp {
  ...
  async getVCById(
    ...
    try {
      ...
      if (credentialSubject && typeof credentialSubject === 'object') {
        ...
        const modifiedCredentialSubject = {
          ...rest,
          ...(end_date && { 'Expiry Date': end_date }),
          ...(issue_date && { 'Issue Date': issue_date }),
        };

        credentialSubject.credentialSubject = modifiedCredentialSubject;
      }

      return {
        statusCode: 200,
        message: `Successfull fetched the VC with ID: ${vcId}`,
        data: {
          id: credential.id,
  ...
  async uploadVCFromQR(
    ...
    try {
      ...
      if (!qrData) {
        throw new Error('Unbale to upload VC from QR: QR data is empty');
      }
